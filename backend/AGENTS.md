...
│   │           ├── doc_parse/         # Document parse pipeline (Docling → batch LLM → JSON)
...
| **Doc Parse** (`/api/document`) | `POST /parse` - multipart (`file`, required `segment_prompt`, optional `output_schema`) → Docling (MarkItDown fallback) → markdown split + batched parallel `run_oneshot_llm` (batch budget from `knowledge.parse` + model `max_tokens`) → merge + grounding warnings → `{data, meta}`. Meta includes `parse_backend`, `warnings`. Auth: `runs:create`. Requires `knowledge` extra (Docling + torch). Code: `deerflow/doc_parse/`, `app/gateway/routers/doc.py`. |
