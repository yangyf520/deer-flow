...
| **Doc Parse** (`/api/document`) | `POST /parse` - multipart (`file`, required `segment_prompt`, optional `output_schema`) → Docling (MarkItDown fallback) → markdown split + batched parallel `run_oneshot_llm` (≤4 concurrent) → merge + grounding warnings → `{data, meta}`. Meta includes `parse_backend`, `warnings`. Auth: `runs:create`. Requires `knowledge` extra (Docling + torch). Code: `deerflow/doc_parse/`, `app/gateway/routers/doc.py`. |
