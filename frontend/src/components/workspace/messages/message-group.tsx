...
function ToolCall({
  ...
  if (name === "web_search") {
    ...
    typeof artifact?.schema_hint === "string" &&
    artifact.schema_hint.length > 0
  ) {
    const schemaHint = artifact.schema_hint;
    const jsonPayload = JSON.stringify(artifact, null, 2);
    return (
      <ChainOfThoughtStep
        key={id}
        label={resolveLabel(t.toolCalls.structuredResult(schemaHint))}
        icon={NotebookPenIcon}
      >
        <details className="group mt-1">
          <summary className="text-muted-foreground cursor-pointer text-xs font-medium">
            {t.toolCalls.viewStructuredResult}
          </summary>
          <CodeBlock
            className="mt-2 max-h-96 overflow-auto border-none"
            showLineNumbers={false}
            language="json"
            code={jsonPayload}
          />
  ...
  for (const [messageIndex, message] of messages.entries()) {
    if (message.type === "ai") {
      ...
      for (const tool_call of message.tool_calls ?? []) {
        ...
        if (toolCallId) {
          const toolMessage = findToolCallMessage(toolCallId, messages);
          if (toolMessage) {
            const toolArtifact = extractToolMessageArtifact(toolMessage);
            if (toolArtifact) {
              step.artifact = toolArtifact;
            }
          }
