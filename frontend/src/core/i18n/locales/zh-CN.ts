...
export const zhCN: Translations = {
  ...
  agents: {
    ...
    soulGenerate: "AI 生成",
    soulGenerateHint:
      "通过对话生成个人设定文本，确认或关闭后写回下方表单（不会创建 SOUL.md 文件）。",
    soulGenerating: "生成中…",
    soulGenerateError: "生成 SOUL 失败",
    soulGenerated: "已生成 SOUL，请检查后保存",
    soulDraftPageTitle: "AI 生成个人设定",
    soulDraftBootstrapMessage:
      "这是一次纯对话任务，为名为 {name} 的智能体设计个人设定（SOUL）。{descriptionPart}请先通过几轮提问收集必要信息；信息足够后，直接在回复里用 ```markdown 代码块输出完整正文。禁止调用任何工具（包括 write_file、read_file、bash、setup_agent、update_agent 等），不要创建、读取或修改 SOUL.md 文件——文本会由界面写回表单。",
    soulDraftDescriptionPart: "功能描述：{description}。",
    soulDraftFinishMessage:
      "请根据我们目前的全部讨论，直接在回复里输出最终个人设定正文。只输出 Markdown，用 ```markdown 代码块包裹。不要调用任何工具，不要读写文件。",
    soulDraftConfirm: "确认 SOUL",
    soulDraftConfirmTitle: "确认个人设定",
    soulDraftConfirmHint: "确认或关闭窗口后写入表单，保存前仍可编辑。",
    soulDraftReturn: "确认并返回",
    soulDraftBackToChat: "继续对话",
    soulDraftChatPlaceholder: "回答智能体的问题，一起完善个人设定…",
    soulDraftExtractError: "未能从回复中提取 SOUL，请继续对话后重试。",
  ...
  chats: {
    ...
    mainChat: "主对话",
    pinnedBadge: "已置顶",
    messageCount: (count) => (count === 1 ? "1 条消息" : `${count} 条消息`),
  },
